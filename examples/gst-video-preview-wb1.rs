...
use rkaiq::prelude::*;
use rkaiq::types::{OpMode, WorkingMode, MWBalanceMode, GetMultiWindowAttrib,AutoWbAttrib};
use std::thread;
...
fn main() {
    ...
    println!("{:?}", out1);
    delay_ms(1000);
    //let out7= ctx.set_mwb_scene(MWBalanceMode::Mode_Scene).unwrap();
    ...
    // 5 秒钟后设置对比度为 255
    delay_ms(2000);//get_autowb_gain
    //let out88= ctx.get_autowb_gain();
      
    //let out5= ctx.get_mwb_gain();
    //println!("{:?}", out88);
    //let out89= ctx.get_multiwindow_attrib();
    //let out89=ctx.get_autowb_attrib();
//get_autowb_attrib
    //let out3= ctx.get_mwb_gain();
    //println!("{:?}", out89);
    //delay_ms(10000);
    ...
    //let out66=ctx.lock_awb();
    let out84= ctx.get_autowb_gain();
